...
IFS=',' read -r -a plugins <<<"$(get_tmux_option "@theme_plugins" "")"
# Default was "datetime,weather" but I disabled it to save battery
# It was sucking battery during long coding sessions with multiple servers, panes and windows
# You can enable if you use PC or always stay plugged in
# To enable: set -g @theme_plugins "datetime,weather" in .tmux.conf
...
tmux set -g status-justify centre # Can use absolute centre insted of center for perfect centering[27;5;106~

### Right side
tmux set-option -g status-right "" #This much is minimal if wanna more just run :64,139s/^#//
#tmux set-option -g status-right "#[bg=dark4, fg=white]#h" #This much is minimal if wanna more just run :64,139s/^#//

# Check if plugins array is empty before proceeding
if [ "$theme_disable_plugins" -ne 1 ] && [ ${#plugins[@]} -gt 0 ]; then
